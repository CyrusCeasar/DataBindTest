...
public class FileExplorerActivity extends Activity implements
            ...
            if (!file.isFile()) {
                ...
                if(file.canRead()) {
                    if (file != mAdapter.getParentFile()) {
                        ...
                        mAdapter.setFiles(mAdapter.getParentFile().getParentFile(),
                                mAdapter.getParentFile());
                    }
                    mAdapter.notifyDataSetChanged();
                    mFileListView.setSelection(0);
                }else{
                    Toast.makeText(FileExplorerActivity.this,"您没权限读取此目录",Toast.LENGTH_SHORT).show();
                }
                return;
            }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        /* if (TextUtils.isEmpty(title)) {
             getTopBarView().setTopBarToStatus(1, R.drawable.topbar_back_bt, -1,
                     R.string.plugin_file_explorer_ui_title, this);
         } else {
    ...
    private void initFileExplorer() {
        ...
        mFileExplorerRootTag = "设备";
        mFileExplorerSdcardTag = "sd卡";
        ...
        File externalStorageFile =  rootDirectory.getParentFile();
      /*  if (!FileUtils.checkExternalStorageCanWrite()) {
            File downloadCacheDirectory = Environment.getDataDirectory();
